			...
			<section id="two" class="wrapper style2 align-center">
				<div class="container">
					<?php
						...
						{
						 	$sql = "SELECT * FROM fproduct WHERE 1 ORDER BY view DESC";
						}
					    if(isset($_GET['type']) AND $_GET['type'] == "fruit")
						{
							$sql = "SELECT * FROM fproduct WHERE pcat = 'Fruit' ORDER BY view DESC";
						}
						if(isset($_GET['type']) AND $_GET['type'] == "vegetable")
						{
							$sql = "SELECT * FROM fproduct WHERE pcat = 'Vegetable' ORDER BY view DESC";
						}
						if(isset($_GET['type']) AND $_GET['type'] == "grain")
						{
							$sql = "SELECT * FROM fproduct WHERE pcat = 'Grains' ORDER BY view DESC";
						}
